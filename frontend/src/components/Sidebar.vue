...
// ELEMENTOS DEL MENÚ
// Ahora usamos componentes de iconos reales, no clases FontAwesome.
const items = [
...
<template>
  <aside :class="[
    ...
    <!-- FONDO DECORATIVO -->
    <div class="absolute -top-24 left-4 h-64 w-64 rounded-full bg-cyan-400/10 blur-3xl"></div>

    <!-- CABECERA -->
    <div class="relative z-10 flex h-24 items-center transition-all duration-300"
      :class="collapsed ? 'justify-center px-0' : 'px-5'">
      <!-- LOGO COMPLETO -->
      <img v-if="!collapsed" src="/logo-isavex.png" alt="ISAVEX"
        class="h-20 md:h-24 w-auto -translate-x-4 object-contain drop-shadow-[0_0_14px_rgba(89,199,216,0.25)]" />
    ...
    <!-- NAVEGACIÓN -->
    <nav class="relative z-10 flex-1 space-y-2 px-2 py-3">
